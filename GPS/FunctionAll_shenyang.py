...
#加cmd
from nav2d_operator import cmd
...
class functionAll:
	 ...
	 #cmd发布
	 self.cmd_pub = rospy.Publisher('cmd', cmd, queue_size=5)
         self.nav_cmd = cmd()
     ...
     def callback2(self, data):
         if self.MapPointNum_Now < self.MapPointNum_All:
		     ...
		     #发布cmd
                     self.nav_cmd.Velocity = self.Motion_temp[0]
	             self.nav_cmd.Turn = -self.Motion_temp[1]
		     #0带壁障控制，1不带
		     self.nav_cmd.Mode = 0
		     self.cmd_pub.publish(celf.nav_cmd)
		     ...
		     #发布cmd
	             self.nav_cmd.Velocity = 0
	             self.nav_cmd.Turn = -self.Motion_temp[2]
		     #0带壁障控制，1不带
		     self.nav_cmd.Mode = 0
		     self.cmd_pub.publish(celf.nav_cmd)
